...
public class AlterRoleGenerator extends AbstractSqlGenerator<AlterRoleStatement> {
  ...
  public ValidationErrors validate(AlterRoleStatement statement, Database database, SqlGeneratorChain chain) {
    ...
    if (statement.getSet() != null) {
      subElementFound++;
      validationErrors.checkRequired("parameter", statement.getSet().getParameter());
      Integer valuesFound = 0;
      if (statement.getSet().getFromCurrent() != null && statement.getSet().getFromCurrent()) {
        valuesFound++;
      }
      if (statement.getSet().getValue() != null && !statement.getSet().getValue().isEmpty()) {
        valuesFound++;
      }
      if (valuesFound != 1) {
